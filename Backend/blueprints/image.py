...
def get_images(filename):
    ...
    try:
        log(f"Accès demandé à l'image de catégorie : {filename}", "info")
        return send_from_directory('asset/category/', filename), 200
    except Exception as e:
        log(f"Erreur lors de l'accès à l'image de catégorie : {filename} : {e}", "error")
...
def get_images_article(filename):
    ...
    try: 
        log(f"Accès demandé à l'image d'article : {filename}", "info")
        return send_from_directory('asset/Picture_doc/', filename), 200
    except Exception as e:
        log(f"Erreur lors de l'accès à l'image d'article : {filename} : {e}", "error")
        return jsonify({'message': 'Image not found'}), 404